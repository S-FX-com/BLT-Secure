...
$blt_login = $options->section( 'login' );
$blt_twofa = $options->section( 'twofa' );
...
<form method="post" action="options.php">
		...
		<tr>
			...
			<td>
				...
				<?php else : ?>
					...
					<input type="text" id="blt-login-slug" name="<?php echo esc_attr( $blt_opt ); ?>[login][slug]" value="<?php echo esc_attr( $blt_login['slug'] ); ?>" class="regular-text" style="max-width:200px;" placeholder="<?php esc_attr_e( 'my-secret-login', 'blt-secure' ); ?>" />
					<p class="description">
	...
	<table class="form-table" role="presentation">
		<tr>
			...
			<td>
				<label><input type="checkbox" name="<?php echo esc_attr( $blt_opt ); ?>[login][lockout_enabled]" value="1" <?php checked( $blt_login['lockout_enabled'] ); ?> />
		...
		<tr>
			...
			<td>
				<input type="number" id="blt-max-attempts" name="<?php echo esc_attr( $blt_opt ); ?>[login][max_attempts]" value="<?php echo esc_attr( $blt_login['max_attempts'] ); ?>" min="3" max="20" class="small-text" />
			</td>
		</tr>
		<tr>
			<th scope="row"><label for="blt-lockout-minutes"><?php esc_html_e( 'Lockout duration (minutes)', 'blt-secure' ); ?></label></th>
			<td>
				<input type="number" id="blt-lockout-minutes" name="<?php echo esc_attr( $blt_opt ); ?>[login][lockout_minutes]" value="<?php echo esc_attr( $blt_login['lockout_minutes'] ); ?>" min="5" max="1440" class="small-text" />
			</td>
		</tr>
	</table>
	...
	<table class="form-table" role="presentation">
		<tr>
			...
			<td>
				<select id="blt-2fa-policy" name="<?php echo esc_attr( $blt_opt ); ?>[twofa][policy]">
					<option value="optional" <?php selected( $blt_twofa['policy'], 'optional' ); ?>><?php esc_html_e( 'Optional (users opt in on their profile)', 'blt-secure' ); ?></option>
					<option value="required_admins" <?php selected( $blt_twofa['policy'], 'required_admins' ); ?>><?php esc_html_e( 'Required for administrators', 'blt-secure' ); ?></option>
					<option value="required_all" <?php selected( $blt_twofa['policy'], 'required_all' ); ?>><?php esc_html_e( 'Required for everyone', 'blt-secure' ); ?></option>
				</select>
				<p class="description">
